...
for cnt = 1:num_MC
    ...
    for percnt = range_percnt
        cntp = cntp+1; disp([cntp,length(range_percnt),cnt,num_MC])
        
        % set baseline diffusion coefficients
        k1=1;k2=1;k3=1;
        ...
        GNPindxf = sort(randperm(vcmplx(3).num(3).val,round(percnt*vcmplx(3).num(3).val)));
        ...
        osf_e = abs(fBO_1)*osf_v;  osf_e(osf_e>1)=1;
        oflux = sum(osf_e.*flux);
